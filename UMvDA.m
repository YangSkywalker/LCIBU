function [W_LDA, W_lda] = UMvDA(X_multiview,Label_multiview,  Ux_multiview, Ul_multiview, eta,d)
...
for vi = 1:num_view
    ...
    Umi = GetEachClass(Ux_multiview{vi},Ul_multiview{vi},'m');
...
%% 
U = zeros(sum(dim), sum(dim));
for j = 1 : num_view
    U_temp = zeros(dim(j), dim(j));
    for iu = 1 : num_class
        for i = 1 : num_class
            U_temp = U_temp + (A{j, 4}(:,iu) - A{j, 2}(:,i)) * (A{j, 4}(:,iu) - A{j, 2}(:,i))';
        end
    end
    U((sum(dim(1 : (j - 1))) + 1) : sum(dim(1 : j)), (sum(dim(1 : (j - 1))) + 1) : sum(dim(1 : j))) = U_temp;
end
%% LDA
W_LDA = LDAX_SwSb(Sw,Sb + eta * U); 
