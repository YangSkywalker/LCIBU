...
Uni = Uni_mean(:, find(b_u >= Threshold));
if size(Uni, 2) > min(numel(ind{1,1}), numel(ind{1,2}))
    [~, ind_Uni] = sort(b_u,'descend');
    Uni = Uni(:, ind_Uni(1:min(numel(ind{1,1}), numel(ind{1,2}))));
end
